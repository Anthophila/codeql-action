...
function initConfig() {
    ...
    const paths = parsedYAML.queries;
    if (paths) {
        paths.forEach(path => {
            ...
            if (tracedLanguageKeys.length > 0) {
                ...
                if (mainTracerConfig.spec) {
                    ...
                    if (process.platform == 'darwin') {
                        ...
                        yield exec.exec('powershell', [path.resolve(__dirname, '..', 'src', 'inject-tracer.ps1'), path.resolve(codeqlSetup.tools, 'win64', 'tracer.exe')], { env: { 'ODASA_TRACER_CONFIGURATION': mainTracerConfig.spec } });
                    }
                    else {

...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (util.should_abort('init', false) || !(yield util.reportActionStarting('init'))) {
                return;
            }
            ...
            const languages = yield util.getLanguages();
            ...
            yield exec.exec(codeqlSetup.cmd, ['version', '--format=json']);
            core.endGroup();
            // Forward Go flags
            ...
            yield io.mkdirP(databaseFolder);
            let tracedLanguages = {};
            let scannedLanguages = [];
            let autobuildLanguages = [];
            ...
            yield util.reportActionFailed('init', error.message, error.stack);
            return;
        }
        core.exportVariable(sharedEnv.CODEQL_ACTION_INIT_COMPLETED, 'true');
        yield util.reportActionSucceeded('init');
    });
}
run().catch(e => {

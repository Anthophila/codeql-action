...
function testHash(input, expectedHashes) {
    var index = 0;
    var callback = function (lineNumber, hash) {
...
test('hash', function () {
    ...
    testHash("Lorem ipsum dolor sit amet.\n".repeat(10), [
        "a7f2ff13bc495cf2:1",
        "a7f2ff13bc495cf2:2",
        "a7f2ff13bc495cf2:3",
        "a7f2ff13bc495cf2:4",
...
function testResolveUriToFile(uri, index, artifactsURIs) {
    var location = { "uri": uri, "index": index };
    var artifacts = artifactsURIs.map(function (uri) { return ({ "location": { "uri": uri } }); });
    return fingerprints.resolveUriToFile(location, artifacts);
}
test('resolveUriToFile', function () {
    ...
    var relativeFilepaht = filepath.substring(cwd.length + 1);
...
test('addFingerprints', function () {
    // Run an end-to-end test on a test file
    var input = fs.readFileSync(__dirname + '/testdata/fingerprinting.input.sarif').toString();
    var expected = fs.readFileSync(__dirname + '/testdata/fingerprinting.expected.sarif').toString();
